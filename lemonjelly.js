function drawLemonjelly(ctx, seed) {
    ...
    let disc = (blob, n) => {
      ...
      if (blob.r2) {
        return {x: blob.x + (blob.rtl == (n == 1) ? 1 : -1) * r, y: blob.y + ((n == 1) ? 1 : -1) * r, r};
      } else {
    ...
    let distance = (disc1, disc2) => {
      return Math.hypot(Math.abs(disc1.x - disc2.x), Math.abs(disc1.y - disc2.y)) - disc1.r - disc2.r;
    };
